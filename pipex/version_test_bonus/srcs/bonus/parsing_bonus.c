...
/*   Updated: 2023/01/19 18:20:44 by mbocquel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_pipes(t_pipex *px)
{
	...
	while (i < px->nb_cmd - 1)
	{
		(px->pipes)[i] = (t_pipe *)malloc(sizeof(t_pipe));
		if ((px->pipes)[i] == NULL)
			ft_exit(px, ERROR_MALLOC, "init_pipes", "(px->pipes)[i]");
		garbage_col(px, (void *)((px->pipes)[i]));
		i++;
	}
}
...
int	parsing_normal(t_pipex *px, int argc, char **argv, char **env)
{
	...
	if (px->pid == NULL)
		ft_exit(px, ERROR_MALLOC, "parsing_normal", "px->pid");
	garbage_col(px, (void *)(px->pid));
	px->pipes = malloc(sizeof(t_pipe *) * (px->nb_cmd - 1));
	if (px->pipes == NULL)
		ft_exit(px, ERROR_MALLOC, "parsing_normal", "px->pipes");
	garbage_col(px, (void *)(px->pipes));
	init_pipes(px);
	return (0);
}
