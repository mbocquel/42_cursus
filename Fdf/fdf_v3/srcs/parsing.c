...
/*   Updated: 2022/12/15 17:30:00 by mbocquel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	process_line(t_map **begin_map, char *line, int num_line)
{
	...
	while (line_tab_char[i])
		i++;
	nb_col = i;
	elem_map = (char **)ft_calloc(nb_col + 1, sizeof(char **));
	if (elem_map == NULL)
		return (0);
	i = -1;
	while (line_tab_char[++i])
		elem_map[i] = line_tab_char[i];
	//free_tab_char(line_tab_char);
	if (lstadd_newline(begin_map, num_line, elem_map, nb_col))
		return (1);
	return (0);
}
