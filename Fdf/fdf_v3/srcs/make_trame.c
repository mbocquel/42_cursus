...
/*   Updated: 2022/12/15 17:19:10 by mbocquel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_trame	*mk_trame_elem(int line, int col, t_map *map)
{
	...
	new->point_3d.z_3d = get_alt_and_color_point(line, col, map, &(new->point_2d_px.color));
	new->point_2d_px.x = 0;
	new->point_2d_px.y = 0;
	/*if (get_color_point(line, col, map))
		new->point_2d_px.color = get_color_point(line, col, map);
	else
